	...
	{//バレット数
		...
		{//使ってる弾
			...
			{//壁の数
				...
				{//使っている透明でない壁
					aPos[0] = D3DXVECTOR3(pMeshWall->pos.x + MESHWALL_WIDTH * 0.5f * pMeshWall->scale.x * sinf(pMeshWall->rot.y + D3DX_PI * -0.5f), pMeshWall->pos.y, pMeshWall->pos.z + MESHWALL_WIDTH * 0.5f * pMeshWall->scale.z * cosf(pMeshWall->rot.y + D3DX_PI * -0.5f));
					aPos[1] = D3DXVECTOR3(pMeshWall->pos.x + MESHWALL_WIDTH * 0.5f * pMeshWall->scale.x * sinf(pMeshWall->rot.y + D3DX_PI * 0.5f), pMeshWall->pos.y, pMeshWall->pos.z + MESHWALL_WIDTH * 0.5f * pMeshWall->scale.z * cosf(pMeshWall->rot.y + D3DX_PI * 0.5f));
					...
					if ((Wallvec.z * Posvec.x) - (Wallvec.x * Posvec.z) < 0.0f && (Wallvec.z * PosOldvec.x) - (Wallvec.x * PosOldvec.z) >= 0.0f && g_aBullet[nCntBullet].pos.y - BULLET_HEIGHT * 0.5f < pMeshWall->pos.y + MESHWALL_HEIGHT * pMeshWall->scale.y && g_aBullet[nCntBullet].pos.y + BULLET_HEIGHT * 0.5f > pMeshWall->pos.y)
					{//壁の外側
			...
			{//壁の数
				...
				{//使っている透明でない壁
					...
					PosOldvec = g_aBullet[nCntBullet].posOld - aPos[0];//壁に対するプレイヤーの旧ベクトル
					movevec = g_aBullet[nCntBullet].pos - g_aBullet[nCntBullet].posOld;//プレイヤーの移動ベクトル
					if ((Wallvec.z * Posvec.x) - (Wallvec.x * Posvec.z) < 0.0f && (Wallvec.z * PosOldvec.x) - (Wallvec.x * PosOldvec.z) >= 0.0f && g_aBullet[nCntBullet].pos.y - BULLET_HEIGHT * 0.5f < pStage->pos.y + pStage->vtxMax.y * pStage->scale.y && g_aBullet[nCntBullet].pos.y + BULLET_HEIGHT * 0.5f > pStage->pos.y + pStage->vtxMin.y)
					{//壁の外側
						WallCross = (Wallvec.z * movevec.x) - (Wallvec.x * movevec.z);
						PosCross = (Posvec.z * movevec.x) - (Posvec.x * movevec.z);
						PosCross /= WallCross;
